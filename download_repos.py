...
def get_search_queries_from_sizes(base_query: str, min_size: int, max_size: int, token: str = None):
    ...
    while True:
        query = f"{base_query} in:file language:python size:{cur_min_size}..{cur_max_size}"
...
def search_github_repos(query, token=None):
    ...
    while True:
        ...
        for item in data.get('items', []):
            ...
            all_repos.add(tuple(repo_info.items()))  # Convert dict to tuple for set storage
...
def find_repos(base_search_query: str, min_size: int, max_size: int, query_dir: str = "triton"):
    # Replace with your GitHub token if you have one
    token = os.getenv("GITHUB_TOKEN")
    # call an error if token is not set
    if not token:
        raise ValueError("GITHUB_TOKEN is not set please run `export GITHUB_TOKEN=<your_token>`")
    ...
    if os.path.exists(f"{file_folder}/github_queries.json"):
        ...
        search_queries = get_search_queries_from_sizes(base_search_query, min_size, max_size, token)
...
def download_repo(repo, download_dir: str = "triton"):
    ...
    if os.path.exists(f"{download_dir}/{org}/{repo_name}"):
        return
    os.system(f"git clone --quiet --filter=blob:limit=5m git@github.com:{org}/{repo_name}.git {download_dir}/{org}/{repo_name} &> /dev/null")
...
def download_repos(repos, query_dir: str = "triton"):
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        list(tqdm(executor.map(lambda repo: download_repo(repo, download_dir), repos), total=len(repos)))
...
def main():
    ...
    if not os.path.exists(f"github_data/{args.query_dir}/github_metadata.json"):
        metadata = get_git_files_metadata(f"github_downloads/{args.query_dir}", args.query)
